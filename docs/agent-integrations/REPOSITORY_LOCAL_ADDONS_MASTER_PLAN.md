# Repository-Local Add-ons Master Plan — pickleball_scoreboard

## Status

**PLANNING ONLY / IMPLEMENT IN A LATER SESSION**

Target repository: `quantdale/pickleball_scoreboard`  
Planning branch: `plan/repo-local-addons-2026-08-28`

## Repository assessment

Hybrid embedded/mobile project: PlatformIO firmware plus Android app, shared display assets, BLE protocol, OpenSpec and existing verification/CI.

**Decision:** `RECOMMEND_MOBILE_DIAGNOSTICS_PLUS_DOCS`

Firmware verification remains PlatformIO/native tests; mobile MCP is only a companion for the Android surface.
...
**Why it fits:** Useful for Android-side BLE UI/device-flow inspection and app lifecycle diagnostics.

**Constraints:** Use test hardware/emulator only; verify patched upstream version because Mobile MCP has had security advisories. It does not replace firmware/native tests or real BLE hardware evidence.

### 2. Context7 MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Current Android/Kotlin and supporting library documentation during BLE app changes.

**Constraints:** Documentation only; no device secrets.

## Explicitly not recommended

- Generic serial/hardware MCP with unrestricted host-device authority
- Cloud/device-farm integration before local BLE qualification is stable
- Global PlatformIO tooling changes
